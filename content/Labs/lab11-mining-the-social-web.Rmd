...
We begin by fetching the basic account information for [[\@CSS\_GMU]{.monospace}][css-gmu-twitter-account], which we will then save to disk for offline access.
We do this so that we don't have to constantly query the Twitter API (which has [`r icon_link`rate limits][twitter-api-rate-limits]) to access the same information.
To query the Twitter API for the account information belonging to the Computational Social Science program and save it, we use:
...
This will write a file named [user\_css\_gmu.rds]{.monospace} in your project directory that contains the information we just collected.
