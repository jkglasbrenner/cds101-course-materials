...
:::::{.callout .secondary}
We will use the [ggplot2]{.monospace} library for all of our visualizations.
You are encouraged to download and use the [official [ggplot2]{.monospace} cheatsheet](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf) for this and future labs.
:::::

## About this week's dataset

You will be exploring the famous dataset by Francis Galton for this week's lab on data visualization, which is automatically loaded into the variable [galton]{.monospace} for you in the RMarkdown file for your lab report.
...
@histogram-fill.
    ...
    What did adding [fill = sex]{.monospace} do?
...
@histogram-analysis.
    Describe the shape of the male and female height distributions and where they seem to be centered around.
...
Also note that each major category, those being [geom\_]{.monospace} objects, [coord\_]{.monospace} objects, [facet\_]{.monospace} objects, [scale\_]{.monospace} objects, and [theme\_]{.monospace} objects, are added to the [ggplot2]{.monospace} "sentence" via the plus sign [+]{.monospace}.
Think of it as a series of layers: first you lay down the canvas ([ggplot]{.monospace}), then you plot the data using a certain type of plot ([geom\_\<function\>]{.monospace}) as the second layer, and afterward you tweak and polish the plot using additional layers to find-tune things.
...
Next, we should try and create a plot that is similar in spirit to what we did in **Exercise @histogram-fill**, so that we can see how the [height]{.monospace} variable depends on the [father]{.monospace} variable when the [sex]{.monospace} variable is taken into account.
